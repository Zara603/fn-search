...
async function createUpdateRequest(
  ...
  return new Promise((resolve, reject) => {
    SoapClient.update('DataExtensionObject', updateValues, updateOptions, (err:any, response:any) => {
      if (err) {
  ...
  return {
    ...
    Properties: {
      Property: [
        ...
        { 'Name': 'continent', 'Value': locationAlert.google_result.continent || "" },
        { 'Name': 'country', 'Value': locationAlert.google_result.country || "" },
        { 'Name': 'administrative_area_level_1', 'Value': locationAlert.google_result.administrative_area_level_1 || "" },
        { 'Name': 'colloquial_area', 'Value': locationAlert.google_result.colloquial_area || "" },
...
async function deleteRequest(id:string, user:IUser): Promise<ISoapResponse> {
  ...
  return new Promise((resolve, reject) => {
    SoapClient.delete('DataExtensionObject', deleteValues, (err:any, response:any) => {
      if (err) {
...
function soapObjectKey(dataObjectName) {
...
export async function createUserLocationAlert(locationAlert:IAlertObject) {
  await createUpdateRequest(locationAlert)
}

export async function updateUserLocationAlert(locationAlert:IAlertObject) {
  await createUpdateRequest(locationAlert)
}

export async function deleteUserLocationAlert(id:string) {
  deleteRequest(id, user) 
}
