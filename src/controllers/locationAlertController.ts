...
export async function createAlert(req: Request, res:Response): Promise<Response | void> {
...
export async function updateAlert(req: Request, res:Response): Promise<Response | void> {
  const errors = schemas.locationAlertSchema.match(req.body);
  if (errors.length) {
    res.status(400);
    logger("error", "Schema Error", errors);
...
export async function deleteAlert(req: Request, res:Response): Promise<Response | void> {
  try {
